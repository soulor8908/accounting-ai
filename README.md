...
- **分期与贷款**：自动生成等额本息 / 等额本金 / 先息后本还款计划，按期还款联动账户余额。
...
- **左滑操作**：账户、流水、记忆统一采用移动端原生左滑手势露出「编辑 / 删除」操作，避免常驻按钮造成的视觉噪音，单行排他打开。
- **流水默认本月**：进入流水页默认筛选当前月份（YYYY-MM），可一键清除查看全部。
- **默认 DeepSeek**：开箱默认指向 DeepSeek（`deepseek-v4-flash`），国内访问稳定、CORS 友好、性价比高；仍可在设置页切换 Agnes 试用 / MiMo / 自定义。
...
# 打包体积分析（生成 dist/stats.html 可视化 treemap）
npm run analyze

# 打包体积守卫（构建后检查 gzip 体积是否超预算）
npm run size
```
...
│   ├── ai/                # AI 客户端、工具、记忆、习惯、Agent
│   ├── analytics/         # 趋势 / 异常 / 导出
│   ├── config/            # 运行期配置（VITE_TRIAL_PROXY_URL 等）
│   ├── engine/            # 指令引擎：AI 输出 → 状态变更
│   ├── finance/           # 分期 / 贷款计算
│   ├── parser/            # 自然语言解析（金额 / 日期 / 分类）
│   ├── security/          # crypto + vault 加密保险库 + 备份还原
│   ├── store/             # 状态仓库（账户/流水/聊天/记忆/快捷输入）
│   ├── utils/             # money / id / now
│   └── types.ts           # 领域类型与 schema 版本
└── ui/                    # React 视图与单例装配
    ├── appState.ts        # Store / Engine / ChatStore 单例
    ├── LockView.tsx       # 加密锁屏（懒加载）
    ├── ChatView.tsx       # AI 对话
    ├── AccountsView.tsx   # 账户管理（左滑编辑/删除）
    ├── TxListView.tsx     # 流水列表（默认本月 + 左滑编辑/删除）
    ├── StatsView.tsx      # 统计
    ├── CalendarView.tsx   # 日历
    ├── SettingsView.tsx   # 设置（AI 配置 / Agent / 记忆，左滑编辑/删除）
    ├── SwipeableRow.tsx   # 通用左滑操作行组件（账户/流水/记忆共用）
    ├── Dialog.tsx         # toast / confirm / prompt 异步弹窗
    ├── FullscreenModal.tsx
    ├── TrendChart.tsx     # 纯 SVG 走势图
    └── useStoreVersion.ts # store 变更驱动的重渲染 hook

scripts/
└── size-guard.mjs         # 打包体积守卫：构建后检查 dist 产物 gzip 体积
```
...
针对 Cloudflare 国内访问慢、对体积敏感的特性，项目内置严格的体积管控：

- **代码分割**：`react-vendor` 单独成 chunk 长期缓存；`LockView` 等次级视图懒加载，首屏仅加载必要代码。
- **打包分析**：`npm run analyze` 生成 `dist/stats.html`（treemap 可视化）。
- **体积守卫**：`npm run size`（[scripts/size-guard.mjs](scripts/size-guard.mjs)）在构建后扫描 `dist/`，对首屏 gzip 总量与各关键 chunk 设定预算（首屏 ≈ 100KB、主包 ≈ 38KB、react-vendor ≈ 47KB、CSS ≈ 10KB），超出即非零退出。
- **CI 集成**：[.github/workflows/ci.yml](.github/workflows/ci.yml) 在 PR 上执行 `lint → test → build → size` 全链路门禁，体积回退会被自动拦截。
- **移除国际化**：为压缩体积，已移除 i18n 模块，文案全部硬编码中文，首屏 gzip 较带 i18n 版本下降约 5KB。
...
- **默认配置**：开箱默认指向 DeepSeek 官方 API（`deepseek-v4-flash`），用户需自备 DeepSeek API Key 直连，无需 Worker。
- **试用模式**：选择「Agnes (内置试用)」时，前端 `apiKey` 为空字符串，仅持有 `proxyUrl`（Worker URL）。
...
- 2026-07-30：性能与体验调优。移除 i18n 模块（硬编码中文，体积下降约 5KB gzip）；
  新增 `scripts/size-guard.mjs` 体积守卫并接入 CI；
  `LockView` 懒加载；账户/流水/记忆三处统一改为左滑卡片露出编辑/删除（`SwipeableRow` 组件）；
  流水页默认筛选当前月份；默认 AI 模型由 Agnes 切换为 DeepSeek（`deepseek-v4-flash`）。
